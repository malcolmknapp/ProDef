# Registers the ProDef language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koProDefLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language ProDef")
    registry.registerLanguage(KoProDefLanguage())

class KoProDefLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "ProDef"
    lexresLangName = "ProDef"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "3ffd2221-084c-4b3c-b611-fa0e1bedf0d7"
    defaultExtension = '.pd'
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'ProDef'}
